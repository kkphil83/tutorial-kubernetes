...
[source,bash,subs="+macros,+attributes"]
----
oc scale sts quarkus-statefulset --replicas=3
----
kbd:[sts] 는 `statefulset` API 리소스의 단축 이름입니다.
...
DNS 이름을 통해 Pod에 액세스하는 기능을 지원하기 위해, kubernetes는 모든 `StatefulSet` Pod에 서비스에 대한 선택기로 사용할 수 있는 레이블을 자동으로 붙입니다.
